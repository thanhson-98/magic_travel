<template>
	<div :class="`dashboard ${sideBarOpen ? '-is-sidebar-visible' : ''} js-dashboard`">
		...
		<div class="dashboard__content">
			...
			<div class="dashboard__content_content">
				...
				<div class="text-center pt-30">© Copyright Viatours {{ new Date().getFullYear() }}</div>
			</div>
		</div>
	</div>
</template>
...
const filterBookingData = () => {
	filteredBookingData.value = bookingData.filter((elm) => elm.status === currentTab.value);
};
...
const handleGetPost = async () => {
	try {
		const res = await Post.getListPost();
	} catch (error) {}
};
onMounted(() => {
	handleGetPost();
